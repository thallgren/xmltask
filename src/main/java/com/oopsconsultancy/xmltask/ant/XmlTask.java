  ...
  private void processDoc(Document doc, final String name) throws BuildException {
    try {
      ...
      for (int r = 0; r < replacements.size(); r++) {
        replacement.add(replacements.get(r));
      }
      ...
      if (dest != null) {
        ...
        if (tobuffer) {
          ...
          // The document must have this attribute set to true or it will always appear in the
          // output, regardless of the OutputKeys.STANDALONE property in the Transformer. By
          // setting it to true, we let the Transformer control the output.
